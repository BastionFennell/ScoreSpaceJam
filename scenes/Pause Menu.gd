...
func _ready():
	...
	get_node("Sections/SFX Volume Section/Volume Slider").connect("value_changed", self, "_sfx_volume_changed")
...
func _sfx_volume_changed(amount):
	var decibels = _get_decibels(amount)
	AudioServer.set_bus_volume_db(AudioServer.get_bus_index("SFX"), decibels)
